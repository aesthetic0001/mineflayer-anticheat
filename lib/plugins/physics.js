...
function inject(bot, {physicsEnabled}) {
  ...
  function updatePosition(dt) {
    ...
    if (performance.now() - lastSent.time >= 1000 && !positionUpdated) {
      // Send a position packet every second, even if no update was made
      if (lookUpdated) sendPacketPositionAndLook(position, yaw, pitch, onGround)
      else sendPacketPosition(position, onGround)
      lastSent.time = performance.now()
    } else if (bot.supportFeature('positionUpdateSentEveryTick') && bot.isAlive && !lookUpdated && !positionUpdated) {
      // For versions < 1.12, one player packet should be sent every tick
      // for the server to update health correctly
      if (performance.now() - lastSent.flyingTime >= 50) {
        bot._client.write('flying', {onGround: bot.entity.onGround})
        lastSent.flyingTime = performance.now()
      }
    }
  }
  ...
  bot._client.on('position', (packet) => {
    ...
    // Velocity is only set to 0 when flag is not set
